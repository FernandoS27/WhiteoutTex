...
void ImageViewer::drawToolbar(SDL_Renderer* renderer) {
    ...
    // ---- Channel filter buttons (right-aligned, hidden in unwrap mode) ----
    if (!show_unwrap_ && is_cube_array_) {
...
void ImageViewer::rebuildPreview(SDL_Renderer* renderer) {
    ...
    // Rebuild unwrap if it was showing.
    if (show_unwrap_)
        buildUnwrapTexture(renderer);
}
...
void ImageViewer::buildUnwrapTexture(SDL_Renderer* renderer) {
    ...
    // Horizontal cross layout (4fw x 3fh, empty corners are black):
    //   col:  0     1     2     3
    // row 0:  .     .    +Y     .
    // row 1: -X    +Z    +X    -Z
    // row 2:  .     .    -Y     .
