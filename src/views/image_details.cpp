...
std::vector<AppCommand> ImageDetails::drawDetailsPanel(tex::Texture* texture,
    ...
    if (texture) {
        ...
        // Pipeline runner first — it's the primary action on a loaded image.
        drawPipelineSection(commands);
...
// ============================================================================
// Pipeline runner (category-grouped list + inline params + Run)
// ============================================================================

void ImageDetails::drawPipelineSection(std::vector<AppCommand>& commands) {
    if (pipelines_.empty())
        return;

    ImGui::SeparatorText(tr("details.pipeline"));

    // Tall list: the pipeline picker is the panel's primary control, so give it
    // generous room (more categories/pipelines visible without scrolling).
    const float list_h = ImGui::GetTextLineHeightWithSpacing() * 12.0f;
    ...
    for (i32 i = 0; i < static_cast<i32>(pipelines_.size()); ++i) {
        ...
        const std::string cat = p.category.empty()
                                    ? std::string(tr("details.pipeline_uncategorized"))
                                    : p.category;
    ...
    if (params) {
        ...
        for (std::size_t i = 0; i < params->size(); ++i) {
            ...
            const std::string shown = !pp.label.empty() ? pp.label
                                      : !pp.name.empty() ? pp.name
                                                         : std::string("value");
            const std::string lbl = shown + "##plp" + std::to_string(i);
